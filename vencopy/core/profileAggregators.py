...
class ProfileAggregator:
    def __init__(self, configDict: dict, activities: pd.DataFrame, profiles: DiaryBuilder):
    ...
    def aggregateProfiles(self):
        ...
        self.chargingPowerWeekly = self.aggregator.performAggregation(
            profile=self.chargingPower, profileName="chargingPower", method="flow"
        )
        self.uncontrolledChargeWeekly = self.aggregator.performAggregation(
            profile=self.uncontrolledCharge, profileName="uncontrolledCharge", method="flow"
        )
        self.maxBatteryLevelWeekly = self.aggregator.performAggregation(
            profile=self.maxBatteryLevel, profileName="maxBatteryLevel", method="state"
        )
        self.minBatteryLevelWeekly = self.aggregator.performAggregation(
            profile=self.minBatteryLevel, profileName="minBatteryLevel", method="state"
        )
...
class Aggregator:
    def __init__(
        self, activities: pd.DataFrame, datasetID: str, alpha: int, user_config: dict, dev_config: dict, weighted: bool
    ):
    ...
    def __basicAggregation(self, byColumn: str = "tripStartWeekday") -> pd.Series:
        ...
        if self.method == "flow":
            ...
            self.__calculateAggregatedStateProfiles(byColumn="tripStartWeekday", alpha=self.alpha)

    def __calculateAverageFlowProfiles(self, byColumn: str):
        for idate in self.activitiesWeekday[byColumn].unique():
            ...
            weekdaySubset = weekdaySubset.drop(columns=["tripStartWeekday", "tripWeight"], axis=1)
    ...
    def __calculateAggregatedStateProfiles(self, byColumn: str, alpha: int = 10):
        ...
        for idate in self.activitiesWeekday[byColumn].unique():
            ...
            weekdaySubset = levels[levels[byColumn] == idate].reset_index(drop=True)
            weekdaySubset = weekdaySubset.drop(columns=["tripStartWeekday", "tripWeight"])
            weekdaySubset = weekdaySubset.convert_dtypes()
            if self.profileName == "maxBatteryLevel":
                self.weekdayProfiles.iloc[idate - 1] = weekdaySubset.quantile(1 - (alpha / 100))
            elif self.profileName == "minBatteryLevel":
                self.weekdayProfiles.iloc[idate - 1] = weekdaySubset.quantile(alpha / 100)
            else:
    ...
    def __composeWeeklyProfile(self):
        ...
        self.weeklyProfile = pd.concat(
            [
                self.weekdayProfiles[1],
                self.weekdayProfiles[2],
                self.weekdayProfiles[3],
                self.weekdayProfiles[4],
                self.weekdayProfiles[5],
                self.weekdayProfiles[6],
                self.weekdayProfiles[7],
            ],
            ignore_index=True,
        )
    ...
    def _writeOutput(self):
        if self.user_config["global"]["writeOutputToDisk"]["aggregatorOutput"]:
            ...
            fileName = createFileName(
                dev_config=self.dev_config,
                user_config=self.user_config,
                manualLabel=self.profileName,
                fileNameID="outputProfileAggregator",
                datasetID=self.datasetID,
            )
            writeOut(data=self.activities, path=root / folder / fileName)

    def performAggregation(self, profile: pd.DataFrame, profileName: str, method: str) -> pd.DataFrame:
