...
class GridModeler:
    def __init__(self, configDict: dict, datasetID: str):
        ...
        self.gridAvailabilitySimple = self.gridConfig[
            "chargingInfrastructureMappings"
        ]
        self.gridAvailabilityProb = self.gridConfig[
            "gridAvailabilityDistribution"
        ]
        self.inputFileName = createFileString(
    ...
    def assignGridViaPurposes(self):
        ...
        """
        print(
            f"Starting with charge connection replacement of location purposes"
        )
        self.chargeAvailability = self.purposeData.replace(
            self.gridAvailabilitySimple
        )
    ...
    def calcGrid(self, grid: str):
        ...
        if grid == "simple":
            ...
            raise (
                ValueError(
                    f"Specified grid modeling option {grid} is not implemented."
                    f"Please choose 'simple' or 'probability'"
                )
            )
